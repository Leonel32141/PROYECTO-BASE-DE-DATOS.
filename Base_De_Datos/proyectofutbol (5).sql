...
-- Tiempo de generación: 07-05-2026 a las 00:01:22
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `jugador_estrella_Boca_Juniors` (IN `club_id` INT)   BEGIN
    SELECT nombre, apellido, goles, asistencias, (goles + asistencias) AS rendimiento_total
    FROM jugadores
    WHERE id_club = 2
...
(1, 1, 1, 40, 15, 25, 15, 3, 1),
(2, 2, 2, 36, 19, 17, 13, 5, 1),
(3, 5, 3, 18, 11, 7, 9, 6, 2),
(4, 4, 4, 22, 22, 0, 8, 3, 9),
...
--
ALTER TABLE `partidos`
  MODIFY `id_partido` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=99;
